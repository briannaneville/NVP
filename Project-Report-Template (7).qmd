...
\begin{aligned}
a_i = \alpha_0 & + u_i \\
& + \alpha_1 (\text{Freq}_{ij}) \\
& + \alpha_2 (\text{Amp}_{ij}) \\
& + \alpha_3 I(\text{Chemical\_Index}_{ij}) \\
&  + \alpha_4 I(\text{Porosity}_{ij})
\end{aligned}
$$

$$
\begin{aligned}
b_i = \beta_0 & + v_i \\ & + \beta_1 I(\text{Chemical\_Index}_{ij})
\end{aligned}
$$
...
- $a_{i}$ and $b_{i}$ are the overall intercept and slope for the model
...
- $u_i$ and $v_i$ are the random intercept and slope. They follow this distribution:
$$
...
- Log(Cycle) × Chemical Index=100: The estimated average rate of change in NVP per increase of one in log(Cycle) is 0.00104 units higher for samples with chemical index 100 than for samples with chemical index 79.
- Log(Cycle) × Chemical Index=121: The estimated average rate of change in NVP per increase of one in log(Cycle) is 0.00066 units higher for samples with chemical index 121 than for samples with chemical index 79.
