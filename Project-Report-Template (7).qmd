...
$$
\begin{aligned}
\text{NVP}_{ij} =\ & a_i + b_i \log(\text{Cycle}_{ij}) + \beta_2 \text{Freq}_{ij} + \beta_3 \text{Amp}_{ij} \\\\
& + \beta_4 \text{ChemIndex}_{ij} + \beta_5 \text{Porosity}_{ij} \\\\
& + \beta_6 \log(\text{Cycle}_{ij}) \times \text{ChemIndex}_{ij} + \epsilon_{ij}
\end{aligned}
$$
...
- $\alpha_{0}$ and $\beta_{0}$ are the global intercept and slope
- $u_{i}$ and $v_{i}$ are the random intercept and slope
...
The model assumptions were reasonably met and the diagnostic plots can be found in the Appendix.
